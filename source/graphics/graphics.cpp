...
namespace chord::graphics
{
	...
	namespace debugUtils
	{
		...
		{
			static auto logger = LoggerSystem::get().registerLogger("GraphicsValidation");
			return *logger;
		}
		...
		static inline auto configMessengerCreateInfo(const Context::InitConfig& config)
		{
			...
			// Config message type.
			if (config.bInstanceLayerValidation) ci.messageType |= VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT;
	...
	bool Context::onInit()
	{
		...
		// Create instance.
		{
			...
			std::vector<const char*> layers{ };
			{
				...
				// Validation layer.
				optionalEnable(m_initConfig.bInstanceLayerValidation, "VK_LAYER_KHRONOS_validation", "Validation");
			}
			...
			std::vector<const char*> extensions{ };
			{
				...
				optionalEnable(m_initConfig.bInstanceExtensionDebugUtils, VK_EXT_DEBUG_UTILS_EXTENSION_NAME, "DebugUtils");

				// GLFW special extension.
				if (m_initConfig.bInstanceExtensionGLFW)
				{
					...
					{
						optionalEnable(m_initConfig.bInstanceExtensionGLFW, glfwExtensions[i], "GLFW");
					}
				}
			}
			...
			{
				LOG_GRAPHICS_TRACE("Instance layer enabled #{0}: '{1}'.", i, layers[i]);
			}

			for (auto i = 0; i < extensions.size(); i++)
			{
				LOG_GRAPHICS_TRACE("Instance extension enabled #{0}: '{1}'.", i, extensions[i]);
			}
			...
			// Instance create info pNext.
			{
				...
				if (m_initConfig.bInstanceExtensionDebugUtils)
				{
		...
		// Choose GPU and query it's properties.
		{
			...
			// Query and cache GPU properties.
			{
				...
				LOG_GRAPHICS_TRACE("Application select GPU '{0}'.", props.deviceProperties.deviceName);
			...
			// Query queue infos.
			{
				...
				for (uint32 queueIndex = 0; queueIndex < queueFamilies.size(); queueIndex ++)
				{
					...
					if (bSupportGraphics && !info.graphicsFamily.isValid())
					{
						...
						LOG_TRACE("Found graphics family at #{0} with count: {1}.", queueIndex, queueFamily.queueCount);
					}
					else if (bSupportCompute && !info.computeFamily.isValid())
					{
						...
						LOG_TRACE("Found compute family at #{0} with count: {1}.", queueIndex, queueFamily.queueCount);
					}
					else if (bSupportCopy && !info.copyFamily.isValid())
					{
						...
						LOG_TRACE("Found copy family at #{0} with count: {1}.", queueIndex, queueFamily.queueCount);
					}
					else if (bSupportSparseBinding && !info.sparseBindingFamily.isValid())
					{
						...
						LOG_TRACE("Found sparse binding family at #{0} with count: {1}.", queueIndex, queueFamily.queueCount);
					}
					else if (bSupportVideoDecode && !info.videoDecodeFamily.isValid())
					{
						...
						LOG_TRACE("Found video decode family at #{0} with count: {1}.", queueIndex, queueFamily.queueCount);
					}
				}
		...
		// Device create.
		{
			std::vector<const char*> deviceExtensionNames{ };
			{
				...
				// Optional device extensions.
				{
					...
					optionalEnable(m_initConfig.b8BitStorage,  VK_KHR_8BIT_STORAGE_EXTENSION_NAME,  "8BitStorage");
					optionalEnable(m_initConfig.b16BitStorage, VK_KHR_16BIT_STORAGE_EXTENSION_NAME, "16BitStorage");
					optionalEnable(m_initConfig.bSwapchain,    VK_KHR_SWAPCHAIN_EXTENSION_NAME,     "Swapchain");
			...
			auto pCreateInfoNext = ChordVkGetNextPtr(enable11GpuFeatures);
			{
				...
				ChordVkStepNextPtr(pCreateInfoNext, enable12GpuFeatures);
				...
				ChordVkStepNextPtr(pCreateInfoNext, enable13GpuFeatures);
				...
				if (m_initConfig.bRaytracing)
				{
					ChordVkStepNextPtr(pCreateInfoNext, accelerationStructureFeatures);
					accelerationStructureFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ACCELERATION_STRUCTURE_FEATURES_KHR;
					accelerationStructureFeatures.accelerationStructure = VK_TRUE;
					accelerationStructureFeatures.accelerationStructureIndirectBuild = VK_TRUE;

					ChordVkStepNextPtr(pCreateInfoNext, raytracingPipelineFeatures);
					raytracingPipelineFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_PIPELINE_FEATURES_KHR;
					raytracingPipelineFeatures.rayTracingPipeline = VK_TRUE;
					raytracingPipelineFeatures.rayTracingPipelineTraceRaysIndirect = VK_TRUE;

					ChordVkStepNextPtr(pCreateInfoNext, rayQueryFeatures);
					rayQueryFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_PIPELINE_FEATURES_KHR;
					rayQueryFeatures.rayQuery = VK_TRUE;
				}

				ChordVkStepNextPtr(pCreateInfoNext, extendedDynamicState3Features);
			...
			std::vector<float> graphicsQueuePriority(m_gpuQueuesInfo.graphcisQueues.size(), 0.5f);
			std::vector<float>   computeQueuePriority(m_gpuQueuesInfo.computeQueues.size(), 0.5f);
			std::vector<float>         copyQueuePriority(m_gpuQueuesInfo.copyQueues.size(), 0.5f);
			std::vector<float> sparseBindingQueuePriority(m_gpuQueuesInfo.spatialBindingQueues.size(), 0.5f);
			std::vector<float> videoDecodeQueuePriority(m_gpuQueuesInfo.videoDecodeQueues.size(), 0.5f);
			...
			VkPhysicalDeviceFeatures2 physicalDeviceFeatures2
			{
				...
				.pNext = &enable11GpuFeatures,
				.features = enable10GpuFeatures
			};
			...
			VkDeviceCreateInfo createInfo
			{
				...
				.pEnabledFeatures = nullptr // We use physical device features2.
			};
