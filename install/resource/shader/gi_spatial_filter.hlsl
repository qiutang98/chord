    ...
    uint2 workGroupId : SV_GroupID, uint localThreadIndex : SV_GroupIndex)
{
    ...
    float4 centerColor =  loadTexture2D_float4(pushConsts.SRV, tid);
    ...
    int filteredRadius = loadTexture2D_float1(pushConsts.originSRV, tid) * 8.0;
    if (filteredRadius == 0)
    {
        storeRWTexture2D_float4(pushConsts.UAV, tid, centerColor);
        return;
    }
